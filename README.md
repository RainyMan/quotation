...
## 報價單欄位與補充說明編輯
- 在品項表格標題的分隔線左右拖曳可調整欄寬，也可將焦點移至分隔線後使用左右方向鍵。輸入完成後，長文字會換行顯示，列印亦完整保留。
- 補充說明保留原本三項預設，每項可獨立輸入；按「＋」在該項下方新增、按「−」刪除、拖曳左側把手排序。全部刪除後仍可使用「新增補充說明」。
- 輸入關鍵字會比對從 PocketBase `memo_presets` 載入的常用說明，點選候選內容即可帶入。儲存報價時沿用原本的自動學習機制。
- 欄寬與補充說明一起存入既有 `memo_html` 欄位，無須變更資料庫欄位；歷史報價、分享檢視與列印均可沿用。部署時須包含新增的 `quotation-editor.js`。


## 案件狀態、請款單與回簽檔案

部署此版本前，請先依 [資料庫設定](DATABASE_SETUP.md) 在 PocketBase 的 quotations 新增 workflow_status、document_type、signed_documents 三個欄位。

歷史紀錄可獨立更新已得標（黃）、請款中（綠）及 ★ 已收款結案，簽名不會覆蓋案件狀態。點擊報價單標題可切換深綠色工程請款單，並同步請款中狀態。內部編輯頁可追加儲存回簽照片或 PDF（每份 20 MiB、最多 10 份），並開啟已保存原檔。
